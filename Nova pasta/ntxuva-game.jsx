...
function countPieces(board, player) {
  const { inner, outer } = PLAYER_ROWS[player];
  return board[inner].reduce((s, p) => s + p, 0)
 ...
 */
function scheduleTone(ctxRef, enabled, freq, dur, type = "sine", vol = 0.25, delay = 0, freqEnd = undefined) {
  if (!enabled) return;
  try {
...
function useSounds(enabled) {
  ...
  const t = useCallback((f, d, tp = "sine", v = 0.25, del = 0, fe = undefined) =>
    scheduleTone(ctxRef, enabled, f, d, tp, v, del, fe), [enabled]);

  return {
    /** Soft click on house selection */
    playSelect:       () => { t(520,0.06,"triangle",0.18); t(380,0.05,"triangle",0.10,0.04); },
    /** Woody knock when a piece is placed */
    playPlace:        () => { t(280,0.10,"triangle",0.22); t(180,0.08,"triangle",0.12,0.04); },
    /** Rising two-note cue for pick-1 continuation */
    playContinuation: () => { t(440,0.10,"sine",0.20); t(587,0.12,"sine",0.18,0.09); },
    /** Sharp impact + rumble on capture */
    playCapture:      () => { t(220,0.04,"sawtooth",0.35); t(160,0.40,"square",0.22,0.03,80); t(320,0.25,"sine",0.18,0.08); t(480,0.15,"sine",0.10,0.18); },
    /** Ascending fanfare on victory */
    playWin:          () => { [523,659,784,1047].forEach((f,i) => t(f,0.30,"sine",0.28,i*0.14)); [523,784].forEach(f => t(f,0.60,"triangle",0.12,4*0.14)); },
    /** Descending lament on defeat */
    playLose:         () => { [392,349,294,220].forEach((f,i) => t(f,0.40,"sine",0.22,i*0.16)); },
    /** Short ping while AI thinks */
    playAIThink:      () => t(330,0.08,"sine",0.10),
  };
}
  ...
  return (
    <div
      ...
      style={{
        ...
        userSelect:"none", minWidth:16, minHeight:16,
      }}
    >
...
function RowLabel({ label, rgb }) {
  return (
    <div style={{ display:"flex", alignItems:"center", gap:8, margin:"3px 0" }}>
      <div style={{ flex:1, height:1, background:`rgba(${rgb},0.22)` }} />
      <span style={{ color:`rgba(${rgb},0.55)`, fontSize:8, letterSpacing:"0.3em", whiteSpace:"nowrap", fontFamily:"monospace" }}>{label}</span>
      <div style={{ flex:1, height:1, background:`rgba(${rgb},0.22)` }} />
    </div>
  );
}
...
function PhaseBadge({ phase, isHuman }) {
  ...
  return (
    <div title={phase===1?"Obrigatório: casa com >1 peça":"Livre: qualquer casa com peça"} style={{
      display:"inline-block",
      background:`rgba(${rgb},0.12)`, border:`1px solid rgba(${rgb},0.35)`,
      borderRadius:10, padding:"2px 8px", fontSize:9, fontWeight:"bold",
      color, letterSpacing:"0.15em", cursor:"help",
    }}>
      F{phase}
    </div>
  );
...
function Board({ board, validMoves, highlighted, capturedCells, lastMoved, busy, onHouseClick }) {
  ...
  const gridStyle = { display:"grid", gridTemplateColumns:`repeat(${cols}, 1fr)`, gap, marginBottom:3 };
  const hasPos = (list, r, c) => list.some(([pr,pc]) => pr===r && pc===c);
    ...
    <div style={gridStyle}>
      {board[rowIdx].map((pieces, col) => (
        <House
          key={col}
          pieces={pieces}
          clickable={!busy && validMoves.some(m => m.row===rowIdx && m.col===col)}
          highlighted={hasPos(highlighted, rowIdx, col)}
          captured={hasPos(capturedCells, rowIdx, col)}
          lastMoved={lastMoved !== null && lastMoved[0]===rowIdx && lastMoved[1]===col}
  ...
  return (
    <div style={{
      ...
      {/* Computer rows (top) */}
      <RowLabel label="← COMPUTADOR DEFESA →" rgb="224,80,40" />
      {renderRow(3, true)}
      <RowLabel label="← COMPUTADOR ATAQUE →" rgb="224,80,40" />
      {renderRow(2, true)}
      ...
      {/* Human rows (bottom) */}
      <RowLabel label="← VOCÊ ATAQUE →" rgb="212,160,32" />
      {renderRow(1, false)}
      <RowLabel label="← VOCÊ DEFESA →" rgb="212,160,32" />
      {renderRow(0, false)}
    </div>
  );
}
...
function useGameLogic(cols, difficulty, soundEnabled) {
  ...
  const addLog = useCallback(t => setLog(p => [t,...p].slice(0,10)), []);

  const applyResult = useCallback((result, pl) => {
    ...
    if (end.over) {
      if (end.winner === 1) { setStatus("won");  setMsg("🏆 Você ganhou! Parabéns!"); sounds.playWin(); }
      else if (end.winner === 0) { setStatus("lost"); setMsg("💀 Computador venceu!"); sounds.playLose(); }
      else { setStatus("tie"); setMsg("🤝 Empate!"); }
      setBusy(false); setHl([]);
      return true;
    }
    return false;
  ...
  const handleHumanMove = useCallback((row, col) => {
    ...
    sounds.playSelect();
    setBusy(true); setHl([[row, col]]); setCapCells([]);
    ...
    setTimeout(() => {
      ...
      if (!ended) {
        setTurn(0); setMsg("Computador pensando…");
        setTimeout(() => { setHl([]); setCapCells([]); setBusy(false); }, 400);
      }
    }, 300);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      if (!aiMove) { setTurn(1); setMsg("Computador sem movimentos. Sua vez!"); return; }
      setBusy(true); setHl([[aiMove.row, aiMove.col]]);
      ...
      setTimeout(() => {
        ...
        if (!ended) {
          setTimeout(() => { setTurn(1); setMsg("Sua vez! Escolha uma casa."); setHl([]); setCapCells([]); setBusy(false); }, 500);
        }
      }, 350);
    }, delay);
  ...
  const resetGame = useCallback(() => {
    setBoard(initBoard(cols)); setTurn(1); setCapH(0); setCapC(0);
    setStatus("playing"); setMsg("Sua vez! Escolha uma casa.");
    setBusy(false); setHl([]); setCapCells([]); setLastMoved(null); setLog([]);
  }, [cols]);

  return {
    board, turn, capH, capC, status,
    validMoves: status==="playing" && turn===1 ? getValidMoves(board,1) : [],
    hl, capCells, lastMoved, msg, busy, log,
  ...
  return (
    <>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr auto 1fr", gap:10, marginBottom:12, alignItems:"center" }}>
        ...
        <div style={{ background:"rgba(224,80,40,0.08)", border:"1px solid rgba(224,80,40,0.2)", borderRadius:10, padding:"10px 14px" }}>
          <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:4 }}>
            ...
            <span style={{ color:"#e05030", fontSize:12, fontWeight:"bold" }}>COMPUTADOR</span>
          </div>
          <PhaseBadge phase={g.compPhase} isHuman={false} />
          <div style={{ color:"#a04020", fontSize:11, marginTop:4 }}>{g.compPieces} peças · {g.capC} cap.</div>
        </div>

        {/* Turn indicator */}
        <div style={{ textAlign:"center", padding:"0 4px" }}>
          <div style={{ width:12, height:12, borderRadius:"50%", margin:"0 auto", background:g.turn===1?"#f0c040":"#e06040", boxShadow:`0 0 12px ${g.turn===1?"rgba(240,192,64,0.9)":"rgba(224,96,64,0.9)"}`, transition:"all 0.3s" }} />
          <div style={{ marginTop:5, fontSize:8, color:"rgba(212,160,32,0.4)", letterSpacing:"0.15em", whiteSpace:"nowrap" }}>
            {g.turn===1?"SEU TURNO":"DELE"}
          </div>
        </div>
        ...
        <div style={{ background:"rgba(212,160,32,0.08)", border:"1px solid rgba(212,160,32,0.2)", borderRadius:10, padding:"10px 14px", textAlign:"right" }}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"flex-end", gap:6, marginBottom:4 }}>
            <span style={{ color:"#d4a020", fontSize:12, fontWeight:"bold" }}>VOCÊ</span>
            <span style={{ fontSize:15 }}>👤</span>
          </div>
          <div style={{ display:"flex", justifyContent:"flex-end", marginBottom:2 }}><PhaseBadge phase={g.humanPhase} isHuman={true} /></div>
          <div style={{ color:"#a07030", fontSize:11 }}>{g.humanPieces} peças · {g.capH} cap.</div>
        </div>
      </div>
      ...
      <div style={{
        ...
        borderRadius:10, color:"#d4a020",
        fontSize:g.status!=="playing"?17:13, fontWeight:g.status!=="playing"?"bold":"normal",
        minHeight:42, display:"flex", alignItems:"center", justifyContent:"center",
      }}>{g.msg}</div>

      {/* Board */}
      <Board board={g.board} validMoves={g.validMoves} highlighted={g.hl}
        capturedCells={g.capCells} lastMoved={g.lastMoved} busy={g.busy}
        onHouseClick={g.handleHumanMove} />

      {/* Move log */}
      <div style={{ background:"rgba(0,0,0,0.2)", border:"1px solid rgba(212,160,32,0.1)", borderRadius:10, padding:"8px 12px", marginTop:10 }}>
        ...
        {g.log.length===0
          ...
          : g.log.map((e,i) => <div key={i} style={{ color:i===0?"#a07030":"#4a3015", fontSize:11, marginBottom:2 }}>{e}</div>)
        }
      </div>
    </>
  );
...
export default function App() {
  ...
  const changeDiff = d => { setDiff(d); bump(); };
  ...
  const btnStyle = (active) => ({
    ...
    borderRadius:6, color:active?"#d4a020":"#7a5025",
    padding:"5px 11px", fontSize:11, cursor:"pointer",
    fontWeight:active?"bold":"normal", transition:"all 0.2s", fontFamily:"inherit",
  });
      ...
      <div style={{ width:"100%", maxWidth:1100, position:"relative", zIndex:1 }}>
        ...
        <div style={{ textAlign:"center", marginBottom:16 }}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:12, marginBottom:4 }}>
            ...
            <h1 style={{ margin:0, fontSize:"clamp(28px,7vw,52px)", fontWeight:900, letterSpacing:"0.25em", color:"#d4a020", textShadow:"0 0 50px rgba(212,160,32,0.35), 0 3px 0 #6b4010" }}>NTXUVA</h1>
        ...
        <div style={{ display:"flex", gap:8, flexWrap:"wrap", alignItems:"center", marginBottom:14 }}>
          <span style={{ color:"#7a5025", fontSize:10, letterSpacing:"0.1em" }}>CASAS:</span>
          {[7,16,22].map(c => <button key={c} onClick={()=>changeCols(c)} style={btnStyle(cols===c)}>{c}</button>)}

          <span style={{ color:"#7a5025", fontSize:10, letterSpacing:"0.1em", marginLeft:10 }}>IA:</span>
          {[["easy","Fácil"],["medium","Médio"],["hard","Difícil"]].map(([d,l]) =>
            <button key={d} onClick={()=>changeDiff(d)} style={btnStyle(diff===d)}>{l}</button>
          )}
          ...
          <button
            ...
            style={{
              marginLeft:10,
              background: soundOn?"rgba(212,160,32,0.14)":"rgba(212,160,32,0.05)",
              border:`1px solid ${soundOn?"rgba(212,160,32,0.5)":"rgba(212,160,32,0.2)"}`,
              borderRadius:8, color:soundOn?"#d4a020":"#5a4015",
              padding:"5px 10px", fontSize:16, cursor:"pointer", transition:"all 0.2s",
            }}
          >{soundOn?"🔊":"🔇"}</button>

          {/* Reset */}
          <button onClick={bump} style={{
            background:"rgba(212,160,32,0.1)", border:"1px solid rgba(212,160,32,0.3)",
            borderRadius:8, color:"#d4a020", padding:"5px 14px",
            fontSize:11, cursor:"pointer", letterSpacing:"0.1em", fontWeight:"bold",
            fontFamily:"inherit", transition:"all 0.2s",
          }}>↺ REINICIAR</button>
        </div>

        <GameView key={gameKey} cols={cols} difficulty={diff} soundEnabled={soundOn} />
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginTop:12 }}>
          ...
          <div style={{ background:"rgba(0,0,0,0.2)", border:"1px solid rgba(212,160,32,0.1)", borderRadius:10, padding:"10px 12px" }}>
            ...
            ].map(r => <div key={r} style={{ color:"#6a4020", fontSize:10, marginBottom:3, lineHeight:1.5 }}>• {r}</div>)}
          </div>
        </div>
